...
// bottommost stair's brick count in every valid stairscse built from K bricks. For example, when 
...
// when there are i bricks and we decide to use j bricks to construct the bottomomost stair, how many
...
// The difficuly is how to get to this definition of dp[i][j]. We can also define dp[i][j] as: count of
//  staircases has i bricks and whose bottommost stair's brick count == j, but then when calculating 
...
// The third solution is focus on space optimization. In solution 2 we kept using a 2 dimensions array, we
// can get: (1) diagonal elements are the finally answers after whole matrix was filled and (2) when calculating
// a column dp[i][j], only its previous column (dp[i-j][j-1]) is used So it is possible to use 1 dimension array.
